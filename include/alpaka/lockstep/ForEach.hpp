...
#    endif
        ...
        public:
            ...
            /** execute a functor
             *
             * Distribute the indices of the domain even over all workers and execute a user defined functor.
             * There is no guarantee in which order the indices will be processed.
             *
             * @param functor is called for each domain index which is mapped to the worker executing that domain index
            ...
            ALPAKA_FN_HOST_ACC ALPAKA_FN_INLINE void operator()(T_Functor&& functor, T_CtxVars&&... ctxVars) const
            {
                ...
                if constexpr(hasRemainder)
                {
                    ...
                    for(uint32_t s = 0u; s < simdSize; ++s)
                    {
                        ...
                        {
                            constexpr uint32_t beginVirtualWorker = peeledIterations * simdSize;
                            uint32_t const beginIdx = beginVirtualWorker * numWorkers + simdSize * this->getWorkerIdx();
                            detail::FunctorWrapper{}(
                                std::forward<T_Functor>(functor),
